...
pal_scp_anomaly <- colorNumeric(c("#B80C09", "#B7B5B3", "#0C2C84"), -100:100,
  na.color = "transparent")

pal_albedo_anomaly <- colorNumeric(c("lightsalmon4", "gray", "lightsalmon"), -1:1,
  na.color = "transparent")
```
...
```{r}
wy2001_monthly_mean_scp_stack_i <- wy2001_monthly_mean_scp_stack[[0]]
leaflet() %>% addTiles() %>%
  addRasterImage(project_area_mask, colors = pal_mask, opacity = 0.5) %>% 
  addRasterImage(wy2001_monthly_mean_scp_stack_i, colors = pal_scp, opacity = 0.8) %>%
  addLegend(pal = pal_scp, values = values(wy2001_monthly_mean_scp_stack_i),
    title = "percent")
```
...
```{r}
january_scp_anomaly_stack <- stack(here("myapp", "data", "snow_cover_percent", "monthly", "january_scp_anomaly_stack.tif"))
january_scp_anomaly_stack
# 20 layers
...
```{r}
leaflet() %>% addTiles() %>%
  addRasterImage(january_scp_anomaly_stack_5, colors = pal_scp_anomaly, opacity = 0.8) %>%
  addLegend(pal = pal_scp_anomaly, values = values(january_scp_anomaly_stack_5),
